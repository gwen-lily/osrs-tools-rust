...
use crate::data::data::Skill::{self, Attack, Defence, Hitpoints, Magic, Ranged, Strength};

/// Structs
///
type Levels = HashMap<Skill, i32>;
type CombatStats = HashMap<Skill, i32>;

fn combat_stats(lvls: Levels) -> CombatStats {
    ...
    for skill in combat_skills.iter() {
        let value: &i32 = lvls.get(skill).expect(msg);
        cs.insert(*skill, *value);
    }
